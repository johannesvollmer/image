...
impl<Buffer> FlatSamples<Buffer> {
    ...
    pub fn as_view<P>(&self) -> Result<View<&[P::Sample], P>, Error>
        where P: Pixel, Buffer: AsRef<[P::Sample]>,
    {
    ...
    /// intended.
    pub fn as_view_with_mut_samples<P>(&mut self) -> Result<View<&mut [P::Sample], P>, Error>
        where P: Pixel, Buffer: AsMut<[P::Sample]>,
    {
        if self.layout.channels != P::CHANNEL_COUNT {
            return Err(Error::WrongColor(P::COLOR_TYPE))
        }
    ...
    pub fn as_view_mut<P>(&mut self) -> Result<ViewMut<&mut [P::Sample], P>, Error>
        where P: Pixel, Buffer: AsMut<[P::Sample]>,
    {
    ...
    Buffer: AsRef<[P::Sample]>
{
    ...
    pub fn get_sample(&self, channel: u8, x: u32, y: u32) -> Option<&P::Sample> {
    ...
    /// **WARNING**: Note that of course samples may alias, so that the mutable reference returned
    /// here can in fact modify more than the coordinate in the argument.
    pub fn get_mut_sample(&mut self, channel: u8, x: u32, y: u32) -> Option<&mut P::Sample>
        where Buffer: AsMut<[P::Sample]>
    {
        if !self.inner.in_bounds(channel, x, y) {
            return None
        }
    ...
    pub fn image_slice(&self) -> &[P::Sample] {
    ...
    pub fn image_mut_slice(&mut self) -> &mut [P::Sample]
        where Buffer: AsMut<[P::Sample]>
    {
    ...
    Buffer: AsMut<[P::Sample]>
{
    ...
    /// While this can not fail–the validity of all coordinates has been validated during the
    /// conversion from `FlatSamples`–the resulting slice may still contain holes.
    pub fn image_slice(&self) -> &[P::Sample] where Buffer: AsRef<[P::Sample]> {
...
impl<Buffer, P: Pixel> GenericImageView for ViewMut<Buffer, P>
    where Buffer: AsMut<[P::Sample]> + AsRef<[P::Sample]>,
{
    type Pixel = P;
